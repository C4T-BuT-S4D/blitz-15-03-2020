...
def create_sample_data(target_config=None):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(orders.insert(), [
            {'id': 2000000001,'user_id': 2000000001, 'flags_id': 2000000001, 'name': '317c6e65-7e38-481a-9227-edbdae51836c',
             'description': '0N3KYAYIAR43482F6EQN60ZH8AIHWVJM=', 'created_date' : datetime.now()},
            {'id': 2000000002,'user_id': 2000000001, 'flags_id': 2000000002, 'name': '88ec2d5a-2330-4181-819a-db2c4d6dd628',
             'description': '423A4JFGKG5MCUCZZUGYE6IQ220IY0UG=', 'created_date' : datetime.now()}
        ])
        conn.execute(transactions.insert(), [
            {'id': 2000000001, 'from_user_id': 2000000001, 'to_user_id': 2000000002, 'msg' : 'ENRVLN3SYOJDRY1JM025RM7P96OSZ08O=',
             'value' : 2, 'created_date' : datetime.now()},
            {'id': 2000000002, 'from_user_id': 2000000002, 'to_user_id': 2000000001, 'msg': '333ZG3JX4BPAWC6JLQHILMLAVY2ABQU4=',
...
async def generate_mysql(loop, executor_config):
    ...
    async with admin_conn.cursor() as cur:
        ...
        try:
            ...
            pass
        try:
            await cur.execute("insert into comments (text, private, author_id) values ('DBH5DM1QTAFNUW3JWPYTO6UZ83X0ZD1Z=', TRUE, (select user_id from users where username='root'))")
        except:
            pass
        try:
            await cur.execute("insert into comments (text, private, author_id) values ('75MPGF37DPBLK22F358W3547FUI0R0DK=', FALSE, (select user_id from users where username='admin'))")
        except:
            pass
