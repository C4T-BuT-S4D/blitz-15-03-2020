...
ACTIONS = ['get_cookie', 'get_cookies', 'get_comments', 'get_my_comments', 'send_comment', 'get_reports', 'get_report', 'create_report']
...
async def validate_action(redis_conn, mongo, mysql_pool, action, data):
    ...
    if action == 'get_cookie':
        ...
        try:
            ...
            result = {'status':'ok', 'private_key' : key, 'object_id' : str(report_id), 'encrypted_text' : base64.encodebytes(encrypted_text).decode().strip('\n')}
        except Exception as ex:
            result = str(ex)
            ...
            async with conn.cursor() as cur:
                try:
                    try:
                        ...
                        pass
                    await cur.execute("insert into comments (text, private, author_id) values ('{}', {}, (select user_id from users where username='{}'))".format(
        ...
        async with mysql_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute('select case private when (username) then username else \'anonymous\' end text, case private when (username) then text else \'private comment\' end text from comments inner join users on author_id=user_id limit 100')
                result =  await cur.fetchall()

    elif action == 'get_my_comments':
        async with mysql_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute('select text from comments where author_id = (select user_id from users where username = %s)', data)
                result = await cur.fetchall()
