...
async def init_mongo(app, loop):
    mongo_uri = "mongodb://{}:{}".format(app['config']['database']['MONGO_HOST'], app['config']['database']['MONGO_PORT'])
...
async def setup_mysql(app, loop):
    pool = await aiomysql.create_pool(host=app['config']['database']['MYSQL_HOST'], port=app['config']['database']['MYSQL_PORT'],
                                      user=app['config']['database']['MYSQL_USER'], password=app['config']['database']['MYSQL_PASSWORD'],
